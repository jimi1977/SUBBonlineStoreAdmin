...
class ProductImagesView extends StatelessWidget {
  ...
  ProductImagesView(this.returnValueFunction, this.productImages);
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer(
        builder: ((context, watch, _) {
          print('Product Image BaseModel..............');
          final model = watch(productImageModel);
                                ...
                                Container(
                                    ...
                                    decoration: BoxDecoration(
                                        ...
                                        image: DecorationImage(
                                            image: productImages.isEmpty || productImages.length <= index
                                                ? AssetImage(
                                                'images/background/white_image.jpg')
                                                : productImages[index].downloadURL == null? FileImage(_displayImage(
                                                model, index)): NetworkImage(productImages[index].downloadURL),
                                            fit: BoxFit.fill)),
                                    //height: double.infinity,

                                    child: OutlineButton(
                                        child: Visibility(
                                          visible: productImages.isEmpty || productImages.length <= index,
                                          child: Icon(
                                            Icons.add,
                                            color: Colors.grey,
                                          ),
                                        ...
                                        onPressed: () async {
                                          if (productImages.isEmpty || productImages.length <= index) {

                                          }
                                          else return;

                                          final _pickedImage = await model.getImage(
                                              source: _imageSource);
                                          model.setState(ViewState.Busy);
